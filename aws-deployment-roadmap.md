...
### Step 2.3: Create the Traces Table

```bash
# Traces table: PK = company_id, SK = run_ts#run_id
# GSI on run_id for single-trace lookup
aws dynamodb create-table \
...
**What this does**:
- Creates the `llmwatch_traces` table used by the agent tracing system
- Partition key: `company_id` (tenant isolation)
- Sort key: `run_ts#run_id` (composite of timestamp + run UUID — enables newest-first listing with `ScanIndexForward=False`)
- GSI `run_id-index`: Used by `get_trace_by_id()` in `dynamo_service.py` for fetching a single trace by its run ID
- Provisioned at 5 RCU/5 WCU (each agent run creates one trace record)
...
aws dynamodb wait table-exists --table-name llmwatch_traces
...
aws dynamodb describe-table --table-name llmwatch_traces --query 'Table.TableStatus'
```

### Step 2.5: Scaling for Production (Later)
    ...
    "Statement": [
        {
            ...
            "Resource": [
                ...
                "arn:aws:dynamodb:us-east-1:*:table/llmwatch_traces",
                "arn:aws:dynamodb:us-east-1:*:table/llmwatch_traces/index/*"
            ]
        },
        {
...
### Step 3.3: Code Changes (Already Done)

The following changes have already been applied to the codebase to support IAM roles on EC2:

- **`backend/config.py`** — `aws_access_key_id` and `aws_secret_access_key` default to `""` so boto3 falls back to the IAM instance role credential chain when keys are not provided.
- **`backend/services/dynamo_service.py`** — Keys are only passed to `boto3.resource()` when non-empty. On EC2 with IAM roles (keys are `""`), boto3 automatically uses the instance metadata credential chain.
- **`frontend/src/api/client.ts`** — `baseURL` uses `import.meta.env.VITE_API_URL || 'http://localhost:8000'` so the API URL is configurable at build time.
- **`frontend/Dockerfile`** — Has `ARG VITE_API_URL` and `ENV VITE_API_URL=$VITE_API_URL` before the build step, so you can pass the backend URL when building the Docker image.

No additional code changes are needed before deployment.
...
# Demo mode — MUST be false on AWS with real DynamoDB
# When true (default), auth endpoints bypass DynamoDB entirely
DEMO_MODE=false
...
# Path-based routing: /auth/*, /chat/*, /analytics/*, /agent/*, /health → backend
aws elbv2 create-rule \
    --listener-arn $LISTENER_ARN \
    --priority 10 \
    --conditions '[{
        "Field": "path-pattern",
        "PathPatternConfig": {"Values": ["/auth/*", "/chat/*", "/analytics/*", "/agent/*", "/health"]}
    }]' \
...
- `http://<ALB_DNS>/agent/run` → Backend (FastAPI)
- `http://<ALB_DNS>/agent/traces` → Backend (FastAPI)
...
# 6. Run an agent task
curl -X POST http://$ALB_DNS/agent/run \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer $TOKEN" \
    -d '{
...
# 7. List agent traces
curl http://$ALB_DNS/agent/traces \
    -H "Authorization: Bearer $TOKEN"
...
# ── Demo Mode ────────────────────────────────────────────
DEMO_MODE=false                             # MUST be false for real DynamoDB auth
...
1. **`backend/config.py`** — AWS keys default to `""` (IAM role compatible) ✅
2. **`backend/services/dynamo_service.py`** — Conditionally passes keys; uses IAM credential chain when empty ✅
3. **`frontend/src/api/client.ts`** — Uses `import.meta.env.VITE_API_URL || 'http://localhost:8000'` ✅
4. **`frontend/Dockerfile`** — Has `ARG VITE_API_URL` / `ENV VITE_API_URL` build args ✅

**No code changes needed — the codebase is deployment-ready.**
...
- [ ] Phase 2: DynamoDB tables (`llmwatch_users` + `llmwatch_logs` + `llmwatch_traces`) created and ACTIVE
...
- [ ] Phase 7: Backend instance launched, Docker container running with production `.env` (`DEMO_MODE=false`)
- [ ] Phase 8: Frontend instance launched, Docker container serving the React build
- [ ] Phase 9: ALB created with path-based routing rules (including `/agent/*`)
- [ ] Phase 10: Full stack verified (register → login → chat → agent → analytics)
