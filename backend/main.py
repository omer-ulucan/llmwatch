...
from routers import auth, chat, analytics
...
async def health_check(request: Request):
    ...
    return {
        ...
        "version": settings.app_version,
        "timestamp": time.time(),
        "uptime_seconds": getattr(app.state, "start_time", time.time()) # Mock uptime
    }

@app.on_event("startup")
async def startup_event():
    app.state.start_time = time.time()
    logger.info(f"LLMWatch {settings.app_version} startup complete in environment: {settings.app_env}")
