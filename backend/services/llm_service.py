...
class LLMStrategy:
    ...
    async def generate(self, prompt: str, thinking_mode: bool) -> Tuple[str, Optional[str], float, int, int]:
        """
...
class QwenStrategy(LLMStrategy):
    ...
    async def generate(self, prompt: str, thinking_mode: bool) -> Tuple[str, Optional[str], float, int, int]:
        ...
        try:
            ...
            return final_response, thinking_content if thinking_mode else None, latency_ms, input_tokens, output_tokens
        except Exception as e:
            raise LLMServiceException(f"Qwen generation failed: {str(e)}", provider="qwen")
...
class GeminiStrategy(LLMStrategy):
    ...
    async def generate(self, prompt: str, thinking_mode: bool) -> Tuple[str, Optional[str], float, int, int]:
        ...
        try:
            ...
            input_tokens = getattr(response, "response_metadata", {}).get("prompt_feedback", {}).get("token_count", len(prompt) // 4)
            output_tokens = getattr(response, "response_metadata", {}).get("candidates", [{}])[0].get("token_count", len(response.content) // 4)

            final_response = response.content
            thinking_content = "Thinking logic computed upstream via Google API." if thinking_mode else None
            
            return final_response, thinking_content, latency_ms, input_tokens, output_tokens
        except Exception as e:
            raise LLMServiceException(f"Gemini generation failed: {str(e)}", provider="gemini")
...
class LLMService:
    ...
    def calculate_cost(self, model: str, input_tokens: int, output_tokens: int) -> float:
        ...
        if model == "qwen":
            ...
            return (input_tokens / 1_000_000) * 0.50 + (output_tokens / 1_000_000) * 3.00
        return 0.0

    async def execute_chat(self, prompt: str, model: str, thinking_mode: bool) -> Dict[str, Any]:
        """
        ...
        response_text, thinking, latency, in_tokens, out_tokens = await strategy.generate(prompt, thinking_mode)
